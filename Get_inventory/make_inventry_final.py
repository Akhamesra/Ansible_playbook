...
def get_inventory(all_instance_data,response_ec2):
    ...
    for instance_info in all_instance_data:
        try:
            ...
            if instance_id not in os_info.keys():
                os_info[instance_id] = {'instance_ip':instance_ip,
                                        'OSName': os_name,
                                        'platversion':platversion}
        except Exception as e:
            print(f"##### get_inventory - ERROR - {e} - {instance_info['InstanceId']}")
        ...
        for instance in reservation['Instances']:
            try:
                ...
                if instance_id in os_info.keys():
                    ...
                    for tag in tags:
                        if tag['Key'] == 'Name':
                            os_info[instance_id]['Name']= tag['Value']
                        elif tag['Key'] == 'LOB':
                            os_info[instance_id]['LOB']= tag['Value']
                else:
                    print(f"##### get_inventory - {instance_id} : instance_id not present in SSM data")
                
            except Exception as e:
                    print(f"##### get_inventory - InstanceID: {instance['InstanceId']} gave the error {e}")
